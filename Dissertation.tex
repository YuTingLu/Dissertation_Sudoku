...
In the annotated instance in cn-view (see Figure \ref{fig:nakedtripletcn}), rows $1$, $3$ and $9$ are the common three rows which appears in the cells $(8,3)$, $(8,8)$, and $(8,9)$ which has shared an unit (column-axis) here. Hence, all other cells which also have rows $1$, $3$ and $9$ in a shared column $8$ could have these three candidates excluded, because these candidates are not longer possible to be the solution for other cells. However, there is no equation has to be re-written because in this unit all other cells are already have the row highlighted.
