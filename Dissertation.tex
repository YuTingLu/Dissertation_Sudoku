...
Naked single is used in a situation that ``there is a cell $A'\ (i,j)$ has only one candidate possibly to be filled into this cell''. If there is one cell has an equation written like $A'\ (i,j) =\ (\set{v}, \set{1,\dots,N} \setminus \set{v})$, then eliminate candidate $v$ from all other cells in the same row, same column, or same block.
...
Hidden single rule is used in a situation that ``there is a cell $A'(i, j)$ has more than one possible candidates, but one candidate $v$ for the cell can only be used in a unit''. If there is any cell $A' (i, j)$ matched the situation, the all other cell in a unit can now eliminate the candidate $v$.

For the annotated instance in rc-view, unit means 
