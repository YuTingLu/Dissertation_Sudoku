...
Each view gives its corresponding informations to each cell on its corresponding x-y axes. For example, row-column view has ``row'' as x-axis, ``number'' as y-axis. This view gives corresponding columns information for each cell in the instance.
...
Figure \ref{fig:rnWhole} is the complete rn-view example which built from rc-view. This provides possibilities that possible columns could have one number filled in in fixed row.
...
Figure \ref{fig:cnWhole} is an extended view made of column and number (called rc-view).
