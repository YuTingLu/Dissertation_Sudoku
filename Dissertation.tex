...
Naked triplet is a rule in the situation that ``three common candidate appear at least two candidates per cell in total three cells in a shared unit''. Then in this shared unit, any other cells has these three candidates in could have these three candidates excluded and rewrite the equation.
A shared unit is varied depend on which view the annotated instance in.
\begin{itemize}
\item If it is an rc-view annotated instance. 
\begin{itemize}
\item shared unit could be a row (x-axis), a column (y-axis), or a block.
\end{itemize}
