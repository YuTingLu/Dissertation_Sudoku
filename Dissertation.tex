...
XXX this is just some highlighting, plus application of the MOST BASIC RULE XXX just looking for the ``asserted'' fields, and removing their numbers from their rows, columns, blocks XXX

If a proper \textbf{Sudoku rule} (which said in section \ref{sec:naturesudokurules}) has been successful applied, certain candidates must have excluded for some cells. When a cell $(i,j)$ in a mapped annotated instance $A'$ has one or more candidates excluded (assume excluded candidate is $e$), the annotation for it has to be rewritten, because the candidate $e$ is no longer possible to be filled to the cell $A'\ (i,j)$. The updated annotation for the cell $A'\ (i,j)$ in an annotated instance $A'$ will be rewritten as:
\begin{itemize}
\item $A'\ (i,j)$ = ($\set{e \notin P}, \set {e \in E})$ (``P'' for ``possible'' and ``E'' for ``excluded'' candidate).
\item all the cells shared a row with $A'\ (i,j)$ have candidate $e$ excluded.
\item all the cells shared a column with $A'\ (i,j)$ have candidate $e$ excluded.
\item all the cells shared a block with $A'\ (i,j)$ have candidate $e$ excluded.
\end{itemize} 

For exmple, in the Figure \ref{fig:rcWhole}
...
\begin{enumerate}
\item Assertion, ??? just highlighting
\item and elimination. ??? happens later!
\end{enumerate}
...
For example, in the Figure \ref{fig:cnWhole},
\begin{itemize}
