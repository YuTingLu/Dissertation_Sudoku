...
Naked pair is in the situation which two cells which shared a unit (such like a row, a column, or a block) has two possibilities to be filled in as an answer. In the Figure \ref{nakedpairb4} which is shown, candidates 5 and 6 are the only two possible candidates in the cell $(r_{1},\ c_{7})$ and $(r_{3},\ c_{7})$. This move is to help eliminating the impossibilities candidates 5 and 6 in other cells which shared a unit (a row, a column, or a block) as that two cells which has only two same candidates in each. After the move, those cells which in the same unit will have candidates 5 and 6 excluded which has a result as Figure \ref{nakedpairaf}. 
