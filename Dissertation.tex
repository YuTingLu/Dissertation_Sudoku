...
To have different perspectives for an instance is aim to help the elimination. Basically, to have an instance built in all different views at the beginning is a time consuming, generate could be made if solving process is blocked somehow. An instance in different perspective might able to provide a solvable instance to find out one or more possible solution to sort out the puzzle. 

The four views that we have introduced are
\begin{enumerate}
...
Each view gives corresponding informations to each cell on its x-y axes. For example, row-column view has ``row'' as x-axis, ``number'' as y-axis. This view gives corresponding columns information for eah cell in the instance.
