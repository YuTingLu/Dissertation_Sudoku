...
\begin{enumerate}
  \item row will be abbreviate to r or row.
  \item column will be abbreviate to c or col.
  \item block will be abbreviate to b, or blk.
\end{enumerate}
...
\end{enumerate}
\[ \forall r\ means\ "for\ all\ row\ r",\]
\[ \forall c\ means\ "for\ all\ column\ c",\]
...
\[ \forall r  \forall n\  \{ \exists !c \  candidate(n, r, c) => value(n, r, c)\}\]
\[ \forall c  \forall n\  \{ \exists !r \  candidate(n, r, c) => value(n, r, c)\}\]
\[ \forall b  \forall n\  \{ \exists !s \  candidate[n, b,s] => value[n, b, s]\}\]
...
\section{Swordfish}
\label{sec:Swordfish}
Swordfish is an extended approach for X-wing which has really rare opportunity to apply on. It is used when, there are three different columns, $c_{1}, c_{2}, c_{3}$, three different rows, $r_{1}, r_{2}, r_{3}$ and a number n in a situation like :
\[r_{1}\ has\ c_{1}\ and\ c_{2}\ among\ its\ candidates,\]
\[r_{2}\ has\ c_{2}\ and\ c_{3}\ among\ its\ candidates,\]
\[r_{3}\ has\ c_{3}\ and\ c_{1}\ among\ its\ candidates,\]
\[and\ non\ of\ r_{1},\ r_{2}\, and\ r_{3}\ has\ other\ candidates\ in\ c_{1},\ c_{2},\ and\ c_{3}.\]
All the candidates for specific number n are occurred in only three specific columns, thus, all candidates for number n which not in these 3 columns could be eliminated immediately. Each defined rows could have 2 or 3 candidates for specific number n.
