...
To get started on Sudoku, the origin and the fundamental mathematic of is have to be studied beforehand. Sudoku is special kind of Latin Square\cite{website:LatinSquare}. Latin square was developed by \textit{Leonhard Euler} in 18th century\footnote{Leonhard Euler, a pioneering Swiss mathematician and physicist. \cite{WikiLeonhardEuler}} which is an $N \times N$ matrices which is standard on a principle: ``All the candidates from 1, \dots , 9 can exact appear once in the unit''. It was named Latin square because the Latin characters are used as symbols to fill into the cells\cite{Delahay2006Science} when it has been developed firstly. Nevertheless, integers could also be used to replace Latin characters. Later on, in 19th, there is a magazine company called ``Dell Magazines'' publish a number puzzle ``Number Place'' with a $9 \times 9$ square grid using Euler's perspective. At that time, it was not that popular till few years later a Japaness puzzle company Nikoli introduced this number puzzle with a new name called ``Sudoku'' again.
...
To puzzle a Sudoku problem, \textit{Denis Berthier} expounds a set of resolution of solving Sudoku puzzle that were already distributed, and the terms to utilize them in the book ``The Hidden Logic of Sudoku\cite{Berthier2007Sudoku}''. All these rules are used when the situation is matched to the prerequisite of the rule. For each cell, the rules will be run one by one until there is one can be applied on, and this procedure will keep running until a final solution has found. A systematic procedure is (from \cite{Berthier2007Sudoku}, Page 24):
\begin{quote}
\begin{tabbing}
...
and the book also issue a concept of providing information in the different representation of Sudoku puzzle. Three other perspectives, row-number, column-number, and block-number are introduced in the book (\cite{Berthier2007Sudoku}, Page 41). No matter which representation (expect block-number view is slightly different) the Sudoku puzzle is displayed in, the informations given are always same information among three factors: row, column, and candidate (number). In fact, three representations are compressed from six different mix-match results as Figure \ref{fig:mixmatch}.
\begin{figure}[h]
