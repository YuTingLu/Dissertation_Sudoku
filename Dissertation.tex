...
The way how the puzzle sequencing its three different units is basically from left to right from top to the bottom. In the puzzle, rows are counted from 1 \dots 9 and sequenced from top to the bottom in the puzzle. But, columns do another way round, which are sequenced from the left to the right.
XXX First of all, there is a must-do task that helps any tech niques application called  \textbf{``pencil-mark''}to be done first of all according the puzzle already had. XXX
